...
jQuery('.vivaco-grid[id^="gridwrapper_"]').each( function() { 

	var $div = jQuery(this);
	var token = $div.data('token');
	var settingObj = window['vsc_grid_' + token];	
	...
	$('#portfolio-wrapper').parents('.wpb_column').addClass('no-padding');
	// Isotope Call
	gridIsotope = function () {
		$container.isotope({
			...
			resizable: false,
			masonry: { columnWidth: colWidth(), gutter: 0, isFitWidth: true }
		});
	};
	gridIsotope();
