var Oldal = {
    ...
    initInstagram : function() {
        $('.insta').simpleInstagramFancybox({
            mode : 'user',
            accessToken : '730091552.5b9e1e6.2966037e3a144c16a57b7a861a1d76d1', // This a mandatory setting that allows you to specify a user token.
    ...
    calculateMainMargin : function() {
        ...
        var height = 0;
        if(mainWrap.length) {
            $('.main-footer').length && (height = $('.main-footer > div').outerHeight());
            $('.simple-footer').length && (height = $('.simple-footer > div').outerHeight());
            $('.main-footer').height(height);
            var width = mainWrap.find('.container').first().width();
            $('.main-footer > div').css({
                left: ($(window).width() - width) / 2 + "px"
            });
            $('.simple-footer > div').css({
                left: ($(window).width() - width) / 2 + "px"
            });
        }
    },
